...
class Patient(object):
    ...
    def update(self):
        ...
        popDensity = len(viruses) / self.maxPop * 1.0

        childViruses = []
        for v in viruses:
            try:
                childViruses.append( v.reproduce(popDensity) )
            except NoChildException:
                pass
        viruses += childViruses
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        popDensity = 1.0 * len(viruses) / self.maxPop

        childViruses = []
        for v in viruses:
            try:
                childViruses.append( v.reproduce(popDensity, self.prescriptions) )
            except NoChildException:
                pass
        viruses += childViruses
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        ...
        for timestep in range(150):
            timestepPop[timestep].append( patient.update() )
            resistPop[timestep].append( patient.getResistPop(['guttagonol']) )
    ...
    meanResistPop = []
    for pop in resistPop:
        mean = sum(pop) / float(numTrials)
        meanResistPop.append(mean)
    
    """print meanPop
    print meanResistPop

    pylab.plot(meanPop, label='Mean Population')
    pylab.plot(meanResistPop, label='Mean Resistant Population')
