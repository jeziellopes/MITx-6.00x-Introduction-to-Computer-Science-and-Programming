...
def fitDataRev(fileName):
    xVals, yVals = getData(fileName)
    xVals = pylab.array(xVals[:-4])
    yVals = pylab.array(yVals[:-4])
    xVals = xVals*9.81  # convert mass to force (F = mg)
    pylab.plot(xVals, yVals, 'bo', label = 'Measured points')
    pylab.title('Measured Displacement of Spring')
    pylab.xlabel('Force (Newtons)')
    ...
    estYValsQ = a*(xVals**2) + b*xVals + c
    pylab.plot(xVals, estYValsQ, 'g',
               label = 'Quadratic Fit' + ', R2 = '
               + str(round(rSquare(yVals, estYValsQ), 4)))
    
    pylab.legend(loc = 'best')
    pylab.legend()

##fitDataRev('springData.txt')
##pylab.show()
